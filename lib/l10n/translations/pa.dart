const Map<String, String> paTranslations = {
  'appTitle': 'ਅਲਾਰਮ',
  'tabWorldClock': 'ਵਿਸ਼ਵ ਘੜੀ',
  'tabAlarm': 'ਅਲਾਰਮ',
  'tabStopwatch': 'ਸਟਾਪਵਾਚ',
  'tabTimer': 'ਟਾਈਮਰ',
  'edit': 'ਸੋਧੋ',
  'done': 'ਹੋ ਗਿਆ',
  'cancel': 'ਰੱਦ ਕਰੋ',
  'delete': 'ਮਿਟਾਓ',
  'back': 'ਪਿੱਛੇ',
  'ok': 'ਠੀਕ ਹੈ',
  'start': 'ਸ਼ੁਰੂ',
  'stop': 'ਬੰਦ',
  'search': 'ਖੋਜੋ',
  'alarms': 'ਅਲਾਰਮ',
  'noAlarm': 'ਕੋਈ ਅਲਾਰਮ ਨਹੀਂ',
  'settings': 'ਸੈਟਿੰਗਾਂ',
  'noAlarmData': 'ਕੋਈ ਅਲਾਰਮ ਡਾਟਾ ਨਹੀਂ',
  'tipKeepAppRunning': 'ਸੁਝਾਅ: ਅਲਾਰਮ ਦੀ ਬਿਹਤਰ ਕਾਰਗੁਜ਼ਾਰੀ ਲਈ ਮਲਟੀਟਾਸਕਿੰਗ ਵਿੱਚ ਐਪ ਨੂੰ ਬੰਦ ਨਾ ਕਰਨ ਦੀ ਸਿਫ਼ਾਰਿਸ਼ ਕੀਤੀ ਜਾਂਦੀ ਹੈ।',
  'days': 'ਦਿਨ',
  'hours': 'ਘੰਟੇ',
  'minutes': 'ਮਿੰਟ',
  'minute': 'ਮਿੰਟ',
  'remaining': 'ਬਾਕੀ',
  'ringingIn': 'ਵੱਜੇਗਾ',
  'alarm': 'ਅਲਾਰਮ',
  'snooze': 'ਸਨੂਜ਼',
  'slideToStop': 'ਬੰਦ ਕਰਨ ਲਈ ਸਲਾਈਡ ਕਰੋ',
  'monday': 'ਸੋਮਵਾਰ',
  'tuesday': 'ਮੰਗਲਵਾਰ',
  'wednesday': 'ਬੁੱਧਵਾਰ',
  'thursday': 'ਵੀਰਵਾਰ',
  'friday': 'ਸ਼ੁੱਕਰਵਾਰ',
  'saturday': 'ਸ਼ਨਿੱਚਰਵਾਰ',
  'sunday': 'ਐਤਵਾਰ',
  'mon': 'ਸੋਮ',
  'tue': 'ਮੰਗਲ',
  'wed': 'ਬੁੱਧ',
  'thu': 'ਵੀਰ',
  'fri': 'ਸ਼ੁੱਕਰ',
  'sat': 'ਸ਼ਨਿ',
  'sun': 'ਐਤ',
  'january': 'ਜਨਵਰੀ',
  'february': 'ਫ਼ਰਵਰੀ',
  'march': 'ਮਾਰਚ',
  'april': 'ਅਪ੍ਰੈਲ',
  'may': 'ਮਈ',
  'june': 'ਜੂਨ',
  'july': 'ਜੁਲਾਈ',
  'august': 'ਅਗਸਤ',
  'september': 'ਸਤੰਬਰ',
  'october': 'ਅਕਤੂਬਰ',
  'november': 'ਨਵੰਬਰ',
  'december': 'ਦਸੰਬਰ',
  'editAlarm': 'ਅਲਾਰਮ ਸੋਧੋ',
  'addAlarm': 'ਅਲਾਰਮ ਸ਼ਾਮਲ ਕਰੋ',
  'deleteAlarm': 'ਅਲਾਰਮ ਮਿਟਾਓ',
  'deleteAlarmConfirm': 'ਕੀ ਤੁਸੀਂ ਯਕੀਨੀ ਤੌਰ \'ਤੇ ਇਹ ਅਲਾਰਮ ਮਿਟਾਉਣਾ ਚਾਹੁੰਦੇ ਹੋ?',
  'repeat': 'ਦੁਹਰਾਓ',
  'sound': 'ਆਵਾਜ਼',
  'snoozeDuration': 'ਸਨੂਜ਼ ਦੀ ਮਿਆਦ',
  'label': 'ਲੇਬਲ',
  'never': 'ਕਦੇ ਨਹੀਂ',
  'everyDay': 'ਹਰ ਰੋਜ਼',
  'weekdays': 'ਹਫ਼ਤੇ ਦੇ ਦਿਨ',
  'weekends': 'ਵੀਕਐਂਡ',
  'none': 'ਕੋਈ ਨਹੀਂ',
  'systemRingtone': 'ਸਿਸਟਮ ਰਿੰਗਟੋਨ',
  'vibrate': 'ਵਾਈਬ੍ਰੇਟ',
  'songs': 'ਗੀਤ',
  'pickASong': 'ਇੱਕ ਗੀਤ ਚੁਣੋ',
  'systemRingtones': 'ਸਿਸਟਮ ਰਿੰਗਟੋਨ',
  'whenTimerEnds': 'ਜਦੋਂ ਟਾਈਮਰ ਖ਼ਤਮ ਹੋਵੇ',
  'worldClock': 'ਵਿਸ਼ਵ ਘੜੀ',
  'noWorldClocks': 'ਕੋਈ ਵਿਸ਼ਵ ਘੜੀ ਨਹੀਂ',
  'chooseACity': 'ਇੱਕ ਸ਼ਹਿਰ ਚੁਣੋ',
  'today': 'ਅੱਜ',
  'timer': 'ਟਾਈਮਰ',
  'resume': 'ਮੁੜ ਸ਼ੁਰੂ',
  'pause': 'ਰੋਕੋ',
  'hoursLabel': 'ਘੰਟੇ',
  'minLabel': 'ਮਿੰਟ',
  'secLabel': 'ਸਕਿੰਟ',
  'lap': 'ਲੈਪ',
  'reset': 'ਰੀਸੈੱਟ',
  'timeFormat': 'ਸਮੇਂ ਦਾ ਫਾਰਮੈਟ',
  'twentyFourHourFormat': '24-ਘੰਟੇ ਫਾਰਮੈਟ',
  'supportAndFeedback': 'ਸਹਾਇਤਾ ਅਤੇ ਫੀਡਬੈਕ',
  'guide': 'ਗਾਈਡ',
  'sendFeedback': 'ਫੀਡਬੈਕ ਭੇਜੋ',
  'about': 'ਬਾਰੇ',
  'shareApp': 'ਐਪ ਸਾਂਝੀ ਕਰੋ',
  'rateApp': 'ਐਪ ਦੀ ਰੇਟਿੰਗ ਕਰੋ',
  'privacyPolicy': 'ਗੋਪਨੀਯਤਾ ਨੀਤੀ',
  'privacyPolicyError': 'ਗੋਪਨੀਯਤਾ ਨੀਤੀ ਖੋਲ੍ਹਣ ਵਿੱਚ ਅਸਮਰੱਥ। ਕਿਰਪਾ ਕਰਕੇ ਸਾਡੀ ਵੈੱਬਸਾਈਟ \'ਤੇ ਜਾਓ।',
  'shareMessage': 'Clock OS 26 - Liquid Glass UI ਡਾਊਨਲੋਡ ਕਰੋ:\nhttps://play.google.com/store/apps/details?id=com.jbh.alarm',
  'alarmNotifications': 'ਅਲਾਰਮ ਸੂਚਨਾਵਾਂ',
  'alarmNotificationsDesc': 'ਅਲਾਰਮ ਲਈ ਸੂਚਨਾਵਾਂ',
  'dismiss': 'ਖਾਰਜ ਕਰੋ',
  'am': 'AM',
  'pm': 'PM',
  'ratingTitle': 'Clock OS 26 ਪਸੰਦ ਆ ਰਿਹਾ ਹੈ?',
  'ratingContent': 'Google Play ਉੱਤੇ ਸਾਨੂੰ ਰੇਟ ਕਰਨ ਲਈ\nਇੱਕ ਤਾਰਾ ਟੈਪ ਕਰੋ.',
  'submit': 'ਜਮ੍ਹਾਂ ਕਰੋ',
  'allowBackgroundRunning': 'ਬੈਕਗ੍ਰਾਊਂਡ ਵਿੱਚ ਚੱਲਣ ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ',
  'batteryDialogContent': '"ਐਪ ਸੈਟਿੰਗਜ਼ ਖੋਲ੍ਹੋ" → ਬੈਟਰੀ → ਬੈਟਰੀ ਵਰਤੋਂ ਦਾ ਪ੍ਰਬੰਧ ਕਰੋ → ਬੈਕਗ੍ਰਾਊਂਡ ਵਿੱਚ ਬੈਟਰੀ ਵਰਤੋਂ ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ ਚੁਣੋ',
  'openAppSettings': 'ਐਪ ਸੈਟਿੰਗਜ਼ ਖੋਲ੍ਹੋ',
  'close': 'ਬੰਦ ਕਰੋ',
  'allowDisplayOverOtherApps': '"ਹੋਰ ਐਪਾਂ ਉੱਤੇ ਦਿਖਾਓ" ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ',
  'overlayDialogContent': '"ਸੈਟਿੰਗਜ਼ ਖੋਲ੍ਹੋ" → Clock OS 26 → ਹੋਰ ਐਪਾਂ ਉੱਤੇ ਦਿਖਾਉਣ ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ ਚੁਣੋ',
  'openSettings': 'ਸੈਟਿੰਗਜ਼ ਖੋਲ੍ਹੋ',
  'guideIntro': 'ਅਲਾਰਮ ਸਹੀ ਤਰ੍ਹਾਂ ਕੰਮ ਕਰਨ ਲਈ, ਅਸੀਂ ਇਹ ਕਦਮ ਅਪਣਾਉਣ ਦੀ ਸਿਫਾਰਸ਼ ਕਰਦੇ ਹਾਂ:',
  'guideStep1Title': '"ਹੋਰ ਐਪਾਂ ਉੱਤੇ ਦਿਖਾਓ" ਇਜਾਜ਼ਤ ਦਿਓ',
  'guideStep1Sub1': 'ਸੈਟਿੰਗਜ਼ → ਐਪਾਂ → Clock OS 26 ਤੇ ਜਾਓ',
  'guideStep1Sub2': '"ਹੋਰ ਐਪਾਂ ਉੱਤੇ ਦਿਖਾਓ" ਚੁਣੋ',
  'guideStep1Sub3': 'ਹੋਰ ਐਪਾਂ ਉੱਤੇ ਦਿਖਾਉਣ ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ',
  'guideStep2Title': 'ਬੈਕਗ੍ਰਾਊਂਡ ਵਿੱਚ ਬੈਟਰੀ ਵਰਤੋਂ ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ',
  'guideStep2Sub1': 'ਸੈਟਿੰਗਜ਼ → ਐਪਾਂ → Clock OS 26 ਤੇ ਜਾਓ',
  'guideStep2Sub2': '"ਐਪ ਬੈਟਰੀ ਵਰਤੋਂ" ਚੁਣੋ',
  'guideStep2Sub3': 'ਬੈਟਰੀ ਵਰਤੋਂ ਦਾ ਪ੍ਰਬੰਧ ਕਰੋ → ਬੈਕਗ੍ਰਾਊਂਡ ਵਿੱਚ ਬੈਟਰੀ ਵਰਤੋਂ ਦੀ ਇਜਾਜ਼ਤ ਦਿਓ',
  'gotIt': 'ਸਮਝ ਗਏ',
};
