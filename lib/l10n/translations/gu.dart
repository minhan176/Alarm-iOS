const Map<String, String> guTranslations = {
  'appTitle': 'એલાર્મ',
  'tabWorldClock': 'વિશ્વ ઘડિયાળ',
  'tabAlarm': 'એલાર્મ',
  'tabStopwatch': 'સ્ટોપવોચ',
  'tabTimer': 'ટાઈમર',
  'edit': 'ફેરફાર',
  'done': 'થઈ ગયું',
  'cancel': 'રદ કરો',
  'delete': 'કાઢી નાખો',
  'back': 'પાછા',
  'ok': 'ઠીક',
  'start': 'શરૂ',
  'stop': 'બંધ',
  'search': 'શોધો',
  'alarms': 'એલાર્મ્સ',
  'noAlarm': 'કોઈ એલાર્મ નથી',
  'settings': 'સેટિંગ્સ',
  'noAlarmData': 'કોઈ એલાર્મ ડેટા નથી',
  'tipKeepAppRunning': 'ટિપ: એલાર્મની સારી કાર્યક્ષમતા માટે મલ્ટિટાસ્કિંગમાં એપ બંધ ન કરવાની ભલામણ કરવામાં આવે છે.',
  'days': 'દિવસો',
  'hours': 'કલાકો',
  'minutes': 'મિનિટો',
  'minute': 'મિનિટ',
  'remaining': 'બાકી',
  'ringingIn': 'વાગશે',
  'alarm': 'એલાર્મ',
  'snooze': 'સ્નૂઝ',
  'slideToStop': 'બંધ કરવા સ્લાઈડ કરો',
  'monday': 'સોમવાર',
  'tuesday': 'મંગળવાર',
  'wednesday': 'બુધવાર',
  'thursday': 'ગુરુવાર',
  'friday': 'શુક્રવાર',
  'saturday': 'શનિવાર',
  'sunday': 'રવિવાર',
  'mon': 'સોમ',
  'tue': 'મંગળ',
  'wed': 'બુધ',
  'thu': 'ગુરુ',
  'fri': 'શુક્ર',
  'sat': 'શનિ',
  'sun': 'રવિ',
  'january': 'જાન્યુઆરી',
  'february': 'ફેબ્રુઆરી',
  'march': 'માર્ચ',
  'april': 'એપ્રિલ',
  'may': 'મે',
  'june': 'જૂન',
  'july': 'જુલાઈ',
  'august': 'ઓગસ્ટ',
  'september': 'સપ્ટેમ્બર',
  'october': 'ઓક્ટોબર',
  'november': 'નવેમ્બર',
  'december': 'ડિસેમ્બર',
  'editAlarm': 'એલાર્મ ફેરફાર કરો',
  'addAlarm': 'એલાર્મ ઉમેરો',
  'deleteAlarm': 'એલાર્મ કાઢી નાખો',
  'deleteAlarmConfirm': 'શું તમે ખરેખર આ એલાર્મ કાઢી નાખવા માંગો છો?',
  'repeat': 'પુનરાવર્તન',
  'sound': 'અવાજ',
  'snoozeDuration': 'સ્નૂઝ અવધિ',
  'label': 'લેબલ',
  'never': 'ક્યારેય નહીં',
  'everyDay': 'દરરોજ',
  'weekdays': 'અઠવાડિયાના દિવસો',
  'weekends': 'સપ્તાહાંત',
  'none': 'કંઈ નહીં',
  'systemRingtone': 'સિસ્ટમ રિંગટોન',
  'vibrate': 'વાઇબ્રેટ',
  'songs': 'ગીતો',
  'pickASong': 'ગીત પસંદ કરો',
  'systemRingtones': 'સિસ્ટમ રિંગટોન્સ',
  'whenTimerEnds': 'જ્યારે ટાઈમર સમાપ્ત થાય',
  'worldClock': 'વિશ્વ ઘડિયાળ',
  'noWorldClocks': 'કોઈ વિશ્વ ઘડિયાળ નથી',
  'chooseACity': 'શહેર પસંદ કરો',
  'today': 'આજે',
  'timer': 'ટાઈમર',
  'resume': 'ફરી શરૂ કરો',
  'pause': 'થોભો',
  'hoursLabel': 'કલાક',
  'minLabel': 'મિનિટ',
  'secLabel': 'સેકન્ડ',
  'lap': 'લેપ',
  'reset': 'રીસેટ',
  'timeFormat': 'સમય ફોર્મેટ',
  'twentyFourHourFormat': '24-કલાક ફોર્મેટ',
  'supportAndFeedback': 'સપોર્ટ અને પ્રતિસાદ',
  'guide': 'માર્ગદર્શિકા',
  'sendFeedback': 'પ્રતિસાદ મોકલો',
  'about': 'વિશે',
  'shareApp': 'એપ શેર કરો',
  'rateApp': 'એપને રેટ કરો',
  'privacyPolicy': 'ગોપનીયતા નીતિ',
  'privacyPolicyError': 'ગોપનીયતા નીતિ ખોલવામાં અસમર્થ. કૃપા કરીને અમારી વેબસાઇટની મુલાકાત લો.',
  'shareMessage': 'Clock OS 26 - Liquid Glass UI ડાઉનલોડ કરો:\nhttps://play.google.com/store/apps/details?id=com.oaptech.clock',
  'alarmNotifications': 'એલાર્મ સૂચનાઓ',
  'alarmNotificationsDesc': 'એલાર્મ માટે સૂચનાઓ',
  'dismiss': 'રદ કરો',
  'am': 'AM',
  'pm': 'PM',
  'ratingTitle': 'Clock OS 26 ગમે છે?',
  'ratingContent': 'Google Play પર અમને રેટ કરવા\nએક સ્ટાર ટેપ કરો.',
  'submit': 'સબમિટ કરો',
  'allowBackgroundRunning': 'બેકગ્રાઉન્ડમાં ચાલવાની મંજૂરી આપો',
  'batteryDialogContent': '"એપ સેટિંગ્સ ખોલો" પસંદ કરો → બેટરી → બેટરી વપરાશ મેનેજ કરો → બેકગ્રાઉન્ડમાં બેટરી વપરાશની મંજૂરી આપો',
  'openAppSettings': 'એપ સેટિંગ્સ ખોલો',
  'close': 'બંધ કરો',
  'allowDisplayOverOtherApps': '"અન્ય એપ્સ ઉપર દર્શાવો" ની મંજૂરી આપો',
  'overlayDialogContent': '"સેટિંગ્સ ખોલો" પસંદ કરો → Clock OS 26 → અન્ય એપ્સ ઉપર દર્શાવવાની મંજૂરી આપો',
  'openSettings': 'સેટિંગ્સ ખોલો',
  'guideIntro': 'એલાર્મ યોગ્ય રીતે કામ કરે તે માટે, અમે આ પગલાં અનુસરવાની ભલામણ કરીએ છીએ:',
  'guideStep1Title': '"અન્ય એપ્સ ઉપર દર્શાવો" ની પરવાનગી આપો',
  'guideStep1Sub1': 'સેટિંગ્સ → એપ્સ → Clock OS 26 પર જાઓ',
  'guideStep1Sub2': '"અન્ય એપ્સ ઉપર દર્શાવો" પસંદ કરો',
  'guideStep1Sub3': 'અન્ય એપ્સ ઉપર દર્શાવવાની મંજૂરી આપો',
  'guideStep2Title': 'બેકગ્રાઉન્ડમાં બેટરી વપરાશની મંજૂરી આપો',
  'guideStep2Sub1': 'સેટિંગ્સ → એપ્સ → Clock OS 26 પર જાઓ',
  'guideStep2Sub2': '"એપ બેટરી વપરાશ" પસંદ કરો',
  'guideStep2Sub3': 'બેટરી વપરાશ મેનેજ કરો → બેકગ્રાઉન્ડમાં બેટરી વપરાશની મંજૂરી આપો',
  'gotIt': 'સમજાયું',
};
