...
class _WorldClockScreenState extends State<WorldClockScreen> {
  ...
  void _addCity() async {
    ...
    if (result != null) {
      if (mounted) {
        Provider.of<WorldClockProvider>(
          context,
          listen: false,
        ).addClock(result);
  ...
  String _getTimeDifference(BuildContext context) {
    try {
      ...
      if (diffHours == 0 && diffMinutes == 0) {
        ...
        String sign = diffDuration.inMinutes > 0
            ? '+'
            : (diffDuration.inMinutes < 0 ? '-' : '');
        if (diffMinutes == 0) {
